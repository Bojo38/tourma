...
public class Generation {
    ...
    public static void generateFirstRound(final int choice) {
        ...
        switch (choice) {
            ...
            case GEN_RANDOM:
                if (!params.mTeamTournament) {
                    generateIndividualFirstRoundOrder(true, false);
                } else {
                    generateTeamFirstRoundOrder(true, false);
                }
                break;
            case GEN_ORDER:
                if (!params.mTeamTournament) {
                    generateIndividualFirstRoundOrder(false, false);
                } else {
                    generateTeamFirstRoundOrder(false, false);
                }
                break;
            case GEN_POOL:
            ...
            case GEN_NAF:
                ...
                if (!params.mTeamTournament) {
                    generateIndividualFirstRoundOrder(false, true);
                } else {
                    generateTeamFirstRoundOrder(false, true);
                }
                break;
            default:
                break;
        ...
        while (nbCounter > 0) {
            ...
            if (response == JOptionPane.YES_OPTION) {
                response = JOptionPane.showConfirmDialog(
                        MainFrame.getMainFrame(),
                        java.text.MessageFormat.format(
                        java.util.ResourceBundle.getBundle("tourma/languages/language").getString("VOULEZ VOUS CHOISIR L'ÉQUIPE DU RANG {0}?"), nbTeams - nbCounter),
            ...
            while (datas2.size() > 0) {
                ...
                for (int i = 0; i < datas2.size(); i++) {
                    ...
                    if ((tour.getParams().mEnableClans) && (tour.getParams().mAvoidClansMatch)) {
                        if (!m.mCoach1.mClan.mName.equals(StringConstants.CS_NONE)) {
            ...
            for (int i = r.getMatchs().size() - 1; i > 0; i--) {
                ...
                if (have_played) {
                    for (int k = i - 1; k >= 0; k--) {
                        ...
                        boolean canMatch = !have_played;
                        if ((tour.getParams().mEnableClans) && (tour.getParams().mAvoidClansMatch)) {
                            if (!r.getMatchs().get(i).mCoach2.mClan.mName.equals(StringConstants.CS_NONE)) {
                        ...
                        if ((tour.getParams().mEnableClans) && (tour.getParams().mAvoidClansMatch)) {
                            if (!r.getMatchs().get(i).mCoach1.mClan.mName.equals(StringConstants.CS_NONE)) {

...
import tourma.JPNCup;
import tourma.JPNRound;
...
public class Generation {
    ...
    public static void NextRound(Round round, int choice, int roundnumber) {
    ...
    protected static void generateFirstRoundPool(final boolean byTeam) {
       ...
       final  ArrayList<Round> rounds = tour.getRounds();
        ...
        Round r;
        boolean random = JOptionPane.showConfirmDialog(MainFrame.getMainFrame(), "Affectation aléatoite (sinon, l'order d'inscription sera utilisée) ?", "Generation", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
        if (random) {
    ...
    protected static void IndivCreatePools() {
        ...
        ArrayList<Pool> pools = tour.getPools();
        ArrayList<Coach> vcoachs = tour.getCoachs();
        

        JLabel jlb = new JLabel("Nombre de joueur par Poule: ");

        JSpinner jspNbPlayers = new JSpinner();
        SpinnerNumberModel model = new SpinnerNumberModel(4, 2, tour.GetActiveCoachNumber() / 2, 2);
        ...
        int nbPools = tour.GetActiveCoachNumber() / nbPlayers;
        ...
        ArrayList<Coach> coachs = new ArrayList<Coach>(tour.GetActiveCoaches());
        while (nbCounter > 0) {
            ...
            if (response == JOptionPane.YES_OPTION) {
                ...
                for (int i = 0; i < nbPools; i++) {
                    int index = ((int) Math.random()) % vcoachs.size();
                    pools.get(i).mCoachs.add(coachs.get(index));
                    coachs.remove(index);
                }
            }
    ...
    protected static void generateIndividualFirstRoundOrder(boolean random) {
        ...
        while ((NotOK) && (counter < 500)) {
            ...
            NotOK = false;
            ArrayList<Coach> shuffle = new ArrayList<Coach>(tour.GetActiveCoaches());
            if (random) /* Aléatoire */ {
    ...
    protected static void generateIndividualFirstRoundRobin() {
        ...
        ArrayList<Coach> coachs = tour.GetActiveCoaches();
        Collections.shuffle(coachs);

        ArrayList<Coach> c1part = new ArrayList<Coach>();
        ArrayList<Coach> c2part = new ArrayList<Coach>();
    ...
    protected static void generateIndividualFirstRoundCup() {
        ...
        int nb_coachs = tour.GetActiveCoachNumber();
        ArrayList<Coach> coachs = new ArrayList<Coach>(tour.GetActiveCoaches());
        Collections.shuffle(coachs);
        int nb_tmp = 1;
        int nbrounds = 0;
        ...
        r.mLooserCup = JOptionPane.showConfirmDialog(MainFrame.getMainFrame(), "Sagit-il d'un tournoi à double élimination ?", "Coupe", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
        ...
        for (int i = 0; i < r.getMatchs().size() / 2; i++) {
            if (i % 2 == 0) {
                Match m1 = r.getMatchs().get(i);
                Match m2 = r.getMatchs().get(r.getMatchs().size() - 1 - i);
    ...
    protected static void generateTeamFirstRoundOrder(boolean random) {
         ...
         */
        ArrayList<Team> shuffle = new ArrayList<Team>(teams);
        if (random) /* Aléatoire */ {
            Collections.shuffle(shuffle);
        }
        for (int i = 0; i
    ...
    protected static void TeamIndivPairing(ArrayList<Team> teams1, ArrayList<Team> teams2, Round r, boolean random) {
        ...
        if (params.mTeamIndivPairing == 1) {
            ...
            for (int i = 0; i
                ...
                ArrayList<Coach> coachs1 = team1.getActivePlayers();
                ArrayList<Coach> shuffle2 = new ArrayList<Coach>(team2.getActivePlayers());
    ...
    protected static void generateTeamFirstRoundRobin() {
        ...
        ArrayList<Team> t1part = new ArrayList<Team>();
        ArrayList<Team> t2part = new ArrayList<Team>();
        ...
        for (int i = 0; i < nb_teamrounds; i++) {
            ...
            ArrayList<Team> teams1 = t1part;
            ArrayList<Team> teams2 = t2part;
            ...
            if (!complete) {
                ...
                for (int n = 0; n < params.mTeamMatesNumber; n++) {
                    ...
                    for (int k = 0; k < teams1.size(); k++) {
                        ...
                        ArrayList<Coach> t1players = team1.getActivePlayers();
                        ArrayList<Coach> t2players = team2.getActivePlayers();
    ...
    protected static void generateTeamFirstRoundManual() {
        ...
        ArrayList<Team> teams = new ArrayList<Team>(vteams);
        while (teams.size() > 0) {
    ...
    protected static void TeamCreatePools() {
        ...
        SpinnerNumberModel model = new SpinnerNumberModel(4, 2, vteams.size() / 2, 2);
        ...
        int nbPools = vteams.size() / nbTeams;
        for (int i = 0; i < nbPools; i++) {
            Pool p = new Pool();
            pools.add(p);
            p.mName = Integer.toString(i + 1);
    ...
    protected static void TeamCupAffectation(Round r, ArrayList<ObjectRanking> datas, int nbTeams) {
    ...
    protected static void TeamCup(Round round, Round r, boolean third_place) {
        Tournament tour = Tournament.getTournament();
        ArrayList<Match> matchs = new ArrayList<Match>(round.getMatchs());
        int nb_match = (int) Math.pow(2, round.mCupMaxTour - round.mCupTour - 1);
        ArrayList<Team> _winners = new ArrayList<Team>();
        ArrayList<Team> _loosers = new ArrayList<Team>();
        Criteria td = tour.getParams().mCriterias.get(0);
        for (int i = 0; i < nb_match; i++) {
            Team t = Match.getTeamMatchWinner(tour.getParams().mTeamMatesNumber, i, matchs);
            Match m = matchs.get(i * tour.getParams().mTeamMatesNumber);
            Team team1 = m.mCoach1.mTeamMates;
            Team team2 = m.mCoach2.mTeamMates;
            if (team1 == t) {
                _winners.add(team1);
                _loosers.add(team2);
            } else {
    ...
    protected static Round IndivSwiss(ArrayList<Coach> coachs, ArrayList<Match> matchs, ArrayList<ObjectRanking> datas, boolean team, Round r) {

        Tournament tour = Tournament.getTournament();
        Calendar cal = Calendar.getInstance();
        r.setHour(cal.getTime());
        ArrayList<ObjectRanking> datas_tmp = new ArrayList<ObjectRanking>(datas);
        for (int i = 0; i < coachs.size(); i++) {
        ...
        if (tour.GetActiveCoaches().size() - 1 <= tour.getRounds().size()) {
            ...
            for (int i = r.getMatchs().size() - 1; i > 0; i--) {
                boolean have_played = false;
                Coach c1 = r.getMatchs().get(i).mCoach1;
                Coach c2 = r.getMatchs().get(i).mCoach2;
                for (int j = 0; j < matchs.size(); j++) {
    ...
    protected static void affectTeamCoachs(Round round, Team team1, Team team2, Round r) {
        Tournament tour = Tournament.getTournament();
        ArrayList<Round> vs = new ArrayList<Round>();
        ...
        for (int j = 0; j < tour.getParams().mTeamMatesNumber; j++) {
            if (tour.getParams().mTeamIndivPairing == 0) {
                ArrayList<ObjectRanking> coachs1 = subRanking(team1.mCoachs, ms, vs);
                ArrayList<ObjectRanking> coachs2 = subRanking(team2.mCoachs, ms, vs);
    ...
    protected static Round IndivQSwiss(ArrayList<Coach> coachs, ArrayList<Match> matchs, ArrayList<ObjectRanking> datas, boolean team, Round r) {
        ...
        ArrayList<ObjectRanking> datas_1 = new ArrayList<ObjectRanking>();
        ArrayList<ObjectRanking> datas_2 = new ArrayList<ObjectRanking>();
        ArrayList<ObjectRanking> datas_3 = new ArrayList<ObjectRanking>();
        ArrayList<ObjectRanking> datas_4 = new ArrayList<ObjectRanking>();
        ...
        if (Tournament.getTournament().GetActiveCoaches().size() - 1 <= tour.getRounds().size()) {
            ...
            ArrayList<ObjectRanking> datas_tmp1 = new ArrayList<ObjectRanking>(datas_1);
            ArrayList<ObjectRanking> datas_tmp2 = new ArrayList<ObjectRanking>(datas_2);
            ...
            ArrayList<ObjectRanking> datas_tmp3 = new ArrayList<ObjectRanking>(datas_3);
            ArrayList<ObjectRanking> datas_tmp4 = new ArrayList<ObjectRanking>(datas_4);
    ...
    protected static Round TeamRandom(ArrayList<Team> teams, ArrayList<Match> matchs, boolean random, boolean free_pairing, ArrayList<Round> rounds, Round r) {

        Tournament tour = Tournament.getTournament();
        Calendar cal = Calendar.getInstance();
        r.setHour(cal.getTime());
        ArrayList<Team> datas = new ArrayList<Team>(teams);
        boolean bOK = false;
        int count = 0;
        while (!bOK) {
            count++;
            ...
            if (count > 100000) {
                ...
                for (int i = 0; i < datas.size() / 2; i++) {
                    ...
                    if (!free_pairing) {
                        if (!random) {
                            ArrayList<ObjectRanking> coachs1 = subRanking(team1.mCoachs, matchs, rounds);
                            ArrayList<ObjectRanking> coachs2 = subRanking(team2.mCoachs, matchs, rounds);
                            ...
                            ArrayList<Coach> coachs1 = new ArrayList<Coach>(team1.mCoachs);
                            Collections.shuffle(coachs1);
                            ArrayList<Coach> coachs2 = new ArrayList<Coach>(team2.mCoachs);
            ...
            if (tour.getTeams().size() - 1 <= tour.getRounds().size()) {
                ...
                bOK = true;
                ArrayList<Team> data_tmp = new ArrayList<Team>(datas);
    ...
    protected static Round IndivRandom(ArrayList<Coach> coachs, ArrayList<Match> matchs, boolean team, Round r) {
        Tournament tour = Tournament.getTournament();
        Calendar cal = Calendar.getInstance();
        r.setHour(cal.getTime());
        ArrayList<Coach> shuffle = new ArrayList<Coach>(coachs);
        boolean bOK = false;
        int count = 0;
        while (!bOK) {
            Collections.shuffle(shuffle);
            ...
            if (Tournament.getTournament().GetActiveCoaches().size() - 1 <= tour.getRounds().size()) {
                ...
                ArrayList<Coach> data_tmp = new ArrayList<Coach>(shuffle);
                bOK = true;
                while (data_tmp.size() > 0) {
                    Match m = new Match();
    ...
    protected static Round TeamSwiss(ArrayList<Team> teams, ArrayList<Match> matchs, ArrayList<ObjectRanking> datas, boolean free_pairing, ArrayList<Round> rounds, Round r) {
    ...
    protected static void IndivCup(Round round, Round r, boolean third_place) {
        ArrayList<Match> matchs = new ArrayList<Match>(round.getMatchs());
        int nb_match = (int) Math.pow(2, round.mCupMaxTour - round.mCupTour - 1);
        ArrayList<Coach> _winners = new ArrayList<Coach>();
        ArrayList<Coach> _loosers = new ArrayList<Coach>();
        if (nb_match == 0) {
            if (round.mCupTour == round.mCupMaxTour + 1) {
    ...
    protected static void IndivCupAffectation(Round r, ArrayList<ObjectRanking> datas, boolean teamTour, int nbPlayers) {
    ...
    protected static Round TeamQSwiss(ArrayList<Team> teams, ArrayList<Match> matchs, ArrayList<ObjectRanking> datas, boolean free_pairing, ArrayList<Round> rounds, Round r) {
        Tournament tour = Tournament.getTournament();
    ...
    protected static void finalGenerationStep(Round round, Round r) {
        Tournament tour = Tournament.getTournament();
        if (round != null) {
            for (int i = 0; i < tour.getRounds().size(); i++) {
    ...
    private static ArrayList<ObjectRanking> subRanking(ArrayList<Coach> coachs, ArrayList<Match> matchs, ArrayList<Round> rounds) {

        Parameters params = Tournament.getTournament().getParams();

        ArrayList<ObjectRanking> result = new ArrayList<ObjectRanking>();
        for (int j = 0; j < coachs.size(); j++) {
    ...
    protected static Round NextRoundQSwiss(Round round, int roundnumber) {
        ...
        // previous rounds
        ArrayList<Round> v = new ArrayList<Round>();
        ...
        if (tour.getPools().isEmpty()) {
            ...
            if (tour.getParams().mTeamTournament) {
                ...
                if (tour.GetActiveCoachNumber() % 2 > 0) {
                    JOptionPane.showMessageDialog(MainFrame.getMainFrame(), "Nombre de coachs actif impair", "Erreur de génération", JOptionPane.WARNING_MESSAGE);
                    return null;
                } else {
    ...
    protected static Round NextRoundCup(Round round, int roundnumber) {
        ...
        if (r.getMatchs().size() < tour.GetActiveCoachNumber() / 2) {
            ...
            if (res == JOptionPane.YES_OPTION) {
                ...
                ArrayList<ObjectRanking> datas = new ArrayList<ObjectRanking>(getSortedRankingData(roundnumber));
    ...
    protected static void buildUntilRound(Round round, ArrayList<Round> v, ArrayList<Match> matchs) {
        Tournament tour = Tournament.getTournament();
        if (round != null) {
            for (int i = 0; i < tour.getRounds().size(); i++) {
                if (tour.getRounds().get(i).getHour().before(round.getHour())) {
    ...
    protected static Round NextRoundSwiss(Round round, int roundnumber) {
    ...
    private static ArrayList<ObjectRanking> getSortedRankingData(int roundnumber) {
        Tournament tour = Tournament.getTournament();
        ArrayList<ObjectRanking> datas;
    ...
    private static ArrayList<ObjectRanking> getSortedRankingData(Pool p, int roundnumber) {
        ArrayList<ObjectRanking> datas;
        Tournament tour = Tournament.getTournament();
        if ((tour.getParams().mTeamTournament) && (tour.getParams().mTeamPairing != 0)) {
            mjtRankingTeam ranking;
    ...
    protected static Round NextRoundRandom(Round round) {
        Tournament tour = Tournament.getTournament();

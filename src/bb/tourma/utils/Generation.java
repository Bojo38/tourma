...
import bb.tourma.JdgCupOptions;
import bb.tourma.JdgManualCupDraw;
import bb.tourma.JdgPairing;
...
import static bb.tourma.languages.Translate.CS_Pool;
...
public final class Generation {
    ...
    private static void generateFirstRoundRobin(ArrayList competitors) {
        ...
        tour.setRoundRobin(false);
        //final Calendar cal = Calendar.getInstance();
        final boolean home_away = (JOptionPane.showConfirmDialog(MainFrame.getMainFrame(), java.util.ResourceBundle.getBundle("bb/tourma/languages/language").getString("MATCH ALLER-RETOUR ?"), java.util.ResourceBundle.getBundle("bb/tourma/languages/language").getString("ALLER-RETOUR"), JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION);
        ...
        if (c2part.get(0) instanceof Team) {
            complete = (JOptionPane.showConfirmDialog(MainFrame.getMainFrame(), java.util.ResourceBundle.getBundle("bb/tourma/languages/language").getString("ROUND ROBIN INTEGRAL (INCLUANT TOUS LES JOUEURS)?"), java.util.ResourceBundle.getBundle("bb/tourma/languages/language").getString("ROUND ROBIN INTEGRAL"), JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION);
        }
     ...
     * @return
     */
    private static Round genSwiss(final ArrayList competitors, final Ranking datas, final Round r, boolean topdown) {
     ...
     */
    @SuppressWarnings("unchecked")
    private static Round genQSwiss(final ArrayList competitors, final Ranking datas, Round r2
    ) {
     ...
     * @param nbPlayers
     */
    private static void genCupFirst(final Round r, final Ranking datas, final int nbPlayers) {
        ...
        if (r != null) {
            if (r.getMatchsCount() < tour.getActiveCoachNumber() / 2) {
                ...
                if (res == JOptionPane.YES_OPTION) {
                    ...
                    Ranking d = getSortedRankingData(roundnumber);
            ...
            if (mix == CATEGORY_DRAW.CATEGORY_RANKING) {
                ...
                for (int i = 0; i < nb_players; i++) {
                    ...
                    if (Tournament.getTournament().getParams().isTeamTournament()
                        ...
                        m = new IndivRanking(roundNumber, Tournament.getTournament().getParams(), sub, Tournament.getTournament().getParams().isTeamTournament(), false, false, false);
                    }
                    for (int j = 0; j < m.getRowCount(); j++) {
                        c.add((Competitor) m.getSortedObject(j).getObject());
                    }
                }
                mjt = new ManualRanking(roundNumber, 0, 0, 0, 0, 0, c, false);

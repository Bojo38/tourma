...
public class AnnexIndivRanking extends AnnexRanking {
    ...
    public AnnexIndivRanking(final int round, final Formula formula, final int subtype, final ArrayList<Coach> coachs, final int ranking_type1, final int ranking_type2, final int ranking_type3, final int ranking_type4, final int ranking_type5, final boolean teamTournament, final boolean round_only) {
        super(round, formula, subtype, coachs, ranking_type1, ranking_type2, ranking_type3, ranking_type4, ranking_type5, round_only);
        mTeamTournament = teamTournament;
        sortDatas();
    }
        ...
        for (int k = 0; k < coaches.size(); k++) {
            ...
            for (int j = 0; j <= c.getMatchCount() - 1; j++) {
                ...
                for (int i = 0; (i < rounds.size()) && (!bFound); i++) {
                    ...
                    if (r == m.getRound()) /*if (r.getCoachMatchs().contains(m))*/ {
                        bFound = true;
                    }
                }
